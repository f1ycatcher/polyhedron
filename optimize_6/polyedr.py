...
class Edge:
    ...
    def shadow(self, facet):
        ...
        if (
            min([self.beg.x, self.fin.x]) > facet.xmax
            or max([self.beg.x, self.fin.x]) < facet.xmin
            or min([self.beg.y, self.fin.y]) > facet.ymax
            or max([self.beg.y, self.fin.y]) < facet.ymin
        ):
            return
